...
3. **生成人工审核报告并打开**（`tools\review_report.html`，同时打开首页预览）。

报告包含五个区块，照着「① 审核清单」从上往下过即可：各页面相对线上版本的**新增/删除明细**（绿增红删）、脚本做过的全部**自动决策记录**（排版修正、链接清理、项目名来源等）、**Word 原始差异**（供对照）。

看过报告没问题后，双击根目录的 `一键上传.cmd` 发布上线。**整个过程不需要 AI 参与。**
